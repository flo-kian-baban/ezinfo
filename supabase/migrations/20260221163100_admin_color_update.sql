-- =========================================================
-- EZinfo: Add theme color columns + update functions
...
-- Step 3: Check which crypt schema your project uses.
-- Run this SELECT first to find out:
--   SELECT proname, pronamespace::regnamespace FROM pg_proc WHERE proname='crypt';
-- Then use the matching schema below.
-- Most Supabase projects use "extensions" (plural).
-- If yours uses "extension" (singular), change "extensions" → "extension" below.

-- Step 4: Replace get_admin_config
CREATE OR REPLACE FUNCTION public.ezinfo_get_admin_config(p_slug text, p_token text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  v_tp RECORD;
BEGIN
  SELECT id, name, admin_token_hash INTO v_biz
  ...
  IF v_biz.admin_token_hash IS NULL
     OR crypt(p_token, v_biz.admin_token_hash) != v_biz.admin_token_hash THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Invalid or missing token');
  END IF;
...
-- Step 5: Replace update_touchpoint
CREATE OR REPLACE FUNCTION public.ezinfo_update_touchpoint(p_slug text, p_token text, p_patch jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  v_tp RECORD;
BEGIN
  SELECT id, admin_token_hash INTO v_biz
  FROM ezinfo.businesses
  WHERE slug = p_slug;
  ...
  VALUES (v_tp.id, 'admin_token_' || p_slug, row_to_json(v_tp)::jsonb);
