    ...
    return (
        <div className="p-6 space-y-6">
            <div className="space-y-1.5 text-center">
                <h2 className="text-[22px] font-bold text-white tracking-tight leading-snug px-2">
            ...
            <div className="pt-2">
                <textarea
                    ...
                    className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-4 text-[15px] text-foreground placeholder:text-muted/30 transition-all resize-none focus:border-accent/40 focus:bg-black/60 focus:outline-none focus:ring-4 focus:ring-accent/10"
                />
            </div>
            ...
            <div className="pt-3 space-y-3">
                {config.ai_enabled && preAiText === null ? (
                    <button
                        ...
                        className={`w-full shadow-lg shadow-black/20 rounded-xl px-4 py-4 text-[15px] font-bold text-white transition-all flex items-center justify-center gap-2 hover:opacity-90 active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed`}
                        style={{ backgroundColor: accentColor }}
                    >
                        {aiLoading ? (
                            <>
                                <svg className="h-5 w-5 animate-spin drop-shadow-sm" viewBox="0 0 24 24" fill="none">
                    ...
                    <button
                        ...
                        className={`w-full shadow-lg shadow-black/20 rounded-xl px-4 py-4 text-[15px] font-bold text-white transition-all flex items-center justify-center gap-2 hover:opacity-90 active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed ${mode === "preview" ? "cursor-default" : ""}`}
                        style={{ backgroundColor: accentColor }}
                    >
                        <svg className="h-5 w-5 drop-shadow-sm" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                ...
                {config.ai_enabled && preAiText === null && (
                    <div className="text-center pt-1.5">
                        <button
                            onClick={handleCopyAndReview}
                            disabled={!reviewText.trim() && !config.google_review_url}
                            className="text-[13px] font-medium text-muted/50 hover:text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                        >
