...
export default function OfferCard({ config, mode, accentColor, logEvent, onToast }: OfferCardProps) {
    ...
    return (
        <div className="relative border-t border-white/5 bg-gradient-to-b from-white/[0.04] to-transparent p-5">
            ...
            <div className="space-y-5">
                <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-4 min-w-0">
                        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-accent/20 shadow-sm" style={{ background: `linear-gradient(135deg, ${accentColor}33, ${accentColor}0D)`, boxShadow: `0 0 15px -3px ${accentColor}80` }}>
                        ...
                        <div className="space-y-1 pt-0.5 min-w-0">
                            <h3 className="text-lg font-extrabold text-white leading-tight truncate tracking-tight">
                ...
                {!offerClaimed ? (
                    <div className="pt-3">
                        <div className={`rounded-xl border w-full overflow-hidden transition-colors duration-300 ${offerFormExpanded ? 'bg-black/40 border-accent/20' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}>
                            <button
                                ...
                                className={`w-full flex items-center justify-between px-4 py-4 text-sm font-bold text-white transition-all active:scale-[0.98] ${offerFormExpanded ? 'bg-accent/5' : ''}`}
                            >
                            ...
                            >
                                <div className="overflow-hidden">
                                    <form onSubmit={handleClaimOffer} className="space-y-4 p-4 pt-2 relative overflow-hidden">
                                        ...
                                        <div className="space-y-3 relative z-10 border-t border-accent/10 pt-4">
                                            <label className="text-xs font-semibold text-muted/70 uppercase tracking-widest pl-1">Where should we send it?</label>
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full rounded-lg border border-white/10 bg-black/40 py-3.5 pl-11 pr-4 text-sm text-white placeholder-muted/50 transition-all duration-300"
                                                />
                                            </div>
                                        </div>

                                        {offerError && (
                                            <p className="text-xs font-semibold text-red-400 text-center bg-red-400/10 py-2 rounded-lg relative z-10">{offerError}</p>
                                        )}

                                        <button
                                            type="submit"
                                            disabled={offerClaiming || !offerEmail}
                                            className="w-full rounded-lg px-4 py-3.5 text-sm font-bold text-white transition-all hover:opacity-90 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none flex items-center justify-center gap-2 relative z-10"
                    ...
                    <div className="pt-4 animate-in fade-in zoom-in-95 duration-500">
                        <div className="relative overflow-hidden rounded-2xl border border-accent/20 p-6 text-center" style={{ background: `linear-gradient(135deg, ${accentColor}1A, transparent)`, boxShadow: `0 0 30px -10px ${accentColor}40` }}>
                            ...
                            <div className="relative z-10 space-y-4">
                                <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-accent/20 border border-accent/30 shadow-inner">
                                    <svg className="h-8 w-8 text-accent drop-shadow-md" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </div>
                                <div className="space-y-1.5">
                                    <h4 className="text-xl font-extrabold tracking-tight text-white mb-1">Here is your offer!</h4>
                                    {config.offer_description && (
                                        <p className="text-base font-bold text-accent break-words">{config.offer_description}</p>
                                    )}
                                </div>
                                {config.offer_terms && (
